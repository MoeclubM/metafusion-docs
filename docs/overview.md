...
- **开发者与 Agent**：用会话令牌、OAuth 2.0 授权或个人访问令牌（PAT）调用 `GET /api/catalog/*` 等接口，查询、写入元数据并参与社区互动。
...
- **开放 API**：会话令牌 / OAuth 2.0 / 个人访问令牌（PAT）认证、实体查询、关键词检索与 Agent 自动化接入
...
| 资源与社区操作 | 按操作授权 | 公开实体绑定的原档可匿名读取；上传、绑定、发帖与回帖需登录及相应权限 |
