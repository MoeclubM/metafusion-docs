...
1. **严禁混淆挂载**：不能把全集盒装的品番/条码挂到其中单部作品名下。
   《千与千寻》只挂它自己的发行版（如 `VWBS-1530`），盒装 `VWBS-1531` 属于盒装发行本身；
   **只有来源证明盒装/汇编本身是独立创作母体时，才另建汇编 Work**。
2. **盒装展开步骤**：
   - 依据来源建立真实发行及其 `subjects`，声明所有实际被收录的 Work；不凭包装名称推造汇编 Work
   - 按实物建有证据的 `medium`（逐盘、标明格式）
   - 各盘 `track` 的 `contents[].expression_id` 指向对应 Work 下可复用的 `expression`
   - 有独立汇编 Work 时才用 `includes` 关联汇编与母作品；若目标实例词表不支持精确 `subjects.role`，报告缺口，不近似冒填
...
比例与分辨率只作为选择清晰、完整图片的展示参考；平台不把固定画幅或最低像素设为通用写入契约。优先使用原始出版方或权利方提供的高清、无水印原图；检查图片清晰度、未被裁切或拉伸，并核实发布/再利用许可或明确的权利方授权依据。官方页面可证明图片和元数据来源，但不自动等于允许再利用；权利和许可不明确时不要使用该图，报告缺口并留空。

- 不使用纯色占位图、404 图、带「暂无图片」水印的过渡图或未经授权的同人图。
- 目录侧 `pictures` 只保存**引用**（URL + 出处），服务端不抓取、不转存；该字段的 `source` 是可核验的来源记录，不替代版权许可。
- 需要长期稳定可引用的图片地址时，把文件交给存储服务后用 `GET /api/storage/assets/:id/content`；对象存储的预签名地址会过期，不适合长期引用。
- `pictures[].source` 应指向具体页面并说明它证明的图片/事实。该 API 只有来源（URL/citation）记录，不提供独立的图片许可字段；许可/授权依据应由贡献者在审查报告中留存，来源链接被接口接受不表示许可已审核。
